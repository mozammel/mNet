...
public class IdGenerator {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            System.out.println(IdGenerator.generate("Dhaka"));
        }
    }

    /* Explicitly mentioning it although it's the default. We need WRITE lock. */
    public static String generate(String divisions) {
        String prefix = divisions.substring(0, 3).toUpperCase();
        String seq = nextSequence(); // the sequence
        String main = "MNET" + "-" + prefix + "-" + seq;
    ...
    private static String nextSequence() {
        ...
        int random = IdGenerator.random.nextInt(3);  // Generate 3 digits at max
